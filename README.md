# Awesome AI Agents 2026

A curated list of AI agent frameworks, tools, and platforms for building autonomous AI systems.

## Agent Frameworks

| Framework | Language | Key Feature |
|-----------|----------|-------------|
| [AutoGen](https://github.com/microsoft/autogen) | Python | Multi-agent conversations |
| [CrewAI](https://github.com/crewAIInc/crewAI) | Python | Role-based agent teams |
| [LangGraph](https://github.com/langchain-ai/langgraph) | Python | Stateful agent graphs |
| [Claude Agent SDK](https://github.com/anthropics/claude-code) | Python/TS | Anthropic's agent framework |
| [Semantic Kernel](https://github.com/microsoft/semantic-kernel) | C#/Python | Enterprise AI orchestration |
| [Haystack](https://github.com/deepset-ai/haystack) | Python | NLP pipelines + agents |
| [Phidata](https://github.com/phidatahq/phidata) | Python | Production AI assistants |
| [Agency Swarm](https://github.com/VRSEN/agency-swarm) | Python | OpenAI-based agent swarms |
| [Pydantic AI](https://github.com/pydantic/pydantic-ai) | Python | Type-safe AI agents |
| [Mastra](https://github.com/mastra-ai/mastra) | TypeScript | TS-first agent framework |

## MCP (Model Context Protocol)

| Tool | Description |
|------|-------------|
| [MCP Specification](https://modelcontextprotocol.io) | Official protocol spec |
| [MCP Servers Collection](https://github.com/spinov001-art/mcp-servers-collection) | 15+ MCP servers curated |
| [MCP TypeScript SDK](https://github.com/modelcontextprotocol/typescript-sdk) | Official TS SDK |
| [MCP Python SDK](https://github.com/modelcontextprotocol/python-sdk) | Official Python SDK |

## Code Agents

| Tool | What It Does |
|------|-------------|
| [Claude Code](https://claude.ai/code) | Anthropic's coding agent |
| [Cursor](https://cursor.sh) | AI-powered code editor |
| [Aider](https://github.com/paul-gauthier/aider) | AI pair programming in terminal |
| [OpenHands](https://github.com/All-Hands-AI/OpenHands) | Open source coding agent |
| [SWE-Agent](https://github.com/princeton-nlp/SWE-agent) | Automated software engineering |
| [Mentat](https://github.com/AbanteAI/mentat) | AI coding assistant |
| [Codex](https://github.com/openai/codex) | OpenAI's coding agent |

## Agent Platforms

| Platform | Best For | Pricing |
|----------|----------|---------|
| [Relevance AI](https://relevanceai.com) | No-code AI agents | Free tier |
| [Flowise](https://github.com/FlowiseAI/Flowise) | Visual LLM flows | Open source |
| [Dify](https://github.com/langgenius/dify) | LLM app development | Open source |
| [n8n](https://n8n.io) | Workflow automation + AI | Open source |
| [Langflow](https://github.com/langflow-ai/langflow) | Visual agent builder | Open source |

## Agent Tools & Utilities

### Memory & State
| Tool | Description |
|------|-------------|
| [Mem0](https://github.com/mem0ai/mem0) | Memory layer for AI agents |
| [Letta](https://github.com/cpacker/MemGPT) | Long-term memory for LLMs |

### Tool Use & Function Calling
| Tool | Description |
|------|-------------|
| [Composio](https://github.com/ComposioHQ/composio) | 150+ tools for AI agents |
| [Toolhouse](https://toolhouse.ai) | Tool use infrastructure |

### Evaluation
| Tool | Description |
|------|-------------|
| [AgentBench](https://github.com/THUDM/AgentBench) | Agent evaluation benchmark |
| [Promptfoo](https://promptfoo.dev) | LLM testing framework |

### Observability
| Tool | Description |
|------|-------------|
| [Langfuse](https://langfuse.com) | Agent tracing & analytics |
| [Arize Phoenix](https://github.com/Arize-ai/phoenix) | LLM observability |

## Learning Resources

- [Building Effective Agents](https://www.anthropic.com/research/building-effective-agents) — Anthropic's guide
- [LLM Powered Autonomous Agents](https://lilianweng.github.io/posts/2023-06-23-agent/) — Lilian Weng's overview
- [Awesome LLM Tools](https://github.com/spinov001-art/awesome-llm-tools) — Production LLM tooling

## Related Lists

- [Awesome LLM Tools](https://github.com/spinov001-art/awesome-llm-tools) — inference, quantization, RAG, deployment
- [Free API Directory](https://github.com/spinov001-art/free-api-directory) — 100+ free APIs for agent tools
- [MCP Servers Collection](https://github.com/spinov001-art/mcp-servers-collection) — curated MCP servers
- [Web Scraping Cheatsheet](https://github.com/spinov001-art/web-scraping-cheatsheet) — data collection for agents
- [Portfolio](https://github.com/spinov001-art) — AI tutorials and guides

## Contributing

Know a tool that's missing? Open an issue or PR!

## License

MIT

## Need Custom AI/Scraping Solutions?

I build production-grade scrapers and AI data pipelines. 78+ tools on [Apify Store](https://apify.com/knotless_cadence).

📧 **[spinov001@gmail.com](mailto:spinov001@gmail.com)** — describe your data need, get a **free quote within 2 hours**.

---

*Curated by [spinov001-art](https://github.com/spinov001-art). Updated April 2026.*
